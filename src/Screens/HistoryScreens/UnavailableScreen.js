...
const UnavailableScreen = ({ uid }) => {
    ...
    useEffect(() => {
        getDataFirstCall(10, "Unavailable", setLoading, setLastDocRef, setCalls, setDataEmpty, setNoResults, uid, Toast)
    }, [])
    ...
    const handleScroll = (nativeEvent) => {
        if (isCloseToBottom(nativeEvent) && calls.length > 0 && !dataEmpty) {
            getDataOnScroll(10, "Unavailable", setScrollLoading, setLastDocRef, setCalls, setDataEmpty, uid, Toast, calls, lastDocRef)
        }
    }
    ...
    const handleSearch = (search) => {
        setDataEmpty(false)
        if (search.length > 2) {
            getDataOnInputSearch(search, uid, "Unavailable", setLastDocRef, setCalls, setNoResults, Toast)
        }
        if (search.length === 0) {
            getDataFirstCall(10, "Unavailable", setLoading, setLastDocRef, setCalls, setDataEmpty, setNoResults, uid, Toast)
        }
...
export default connect(mapStateToProps)(UnavailableScreen)