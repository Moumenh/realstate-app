...
const CallBackScreen = ({ uid }) => {
    ...
    useEffect(() => {
        getDataFirstCall(15, "CallBack", setLoading, setLastDocRef, setCalls, setDataEmpty, setNoResults, uid, Toast)
    }, [])
    ...
    const handleScroll = (nativeEvent) => {
        if (isCloseToBottom(nativeEvent) && calls.length > 0 && !dataEmpty) {
            getDataOnScroll(15, "CallBack", setScrollLoading, setLastDocRef, setCalls, setDataEmpty, uid, Toast, calls, lastDocRef)
        }
    }
    ...
    const handleSearch = (search) => {
        setDataEmpty(false)
        if (search.length > 2) {
            getDataOnInputSearch(search, uid, "CallBack", setLastDocRef, setCalls, setNoResults, Toast)
        }
        if (search.length === 0) {
            getDataFirstCall(10, "CallBack", setLoading, setLastDocRef, setCalls, setDataEmpty, setNoResults, uid, Toast)
        }
    }
