...
const RegisterNewAgency = ({ navigation }) => {
    ...
    const assignEmail = async () => {
        ...
        setLoading(true)
        await db.collection('VerifiedOwners').doc(email.toLocaleLowerCase()).set({
            email,
            role: 2
        })
        ...
        <ScrollView style={{flex: 1}}  keyboardShouldPersistTaps='handled' contentContainerStyle={{ flexGrow: 1 }}>
            <View style={styles.container}>
                <View style={styles.loginContainer}>
                    <Text style={styles.loginTitle}>Register new agency </Text>
                    <View style={{ flex: 0.45 }}>

                        <Input onChangeText={(email) => setEmail(email)} placeholder='Agency Email' iconName='email' iconType='Fontisto' type="email-address" />
                    </View>
